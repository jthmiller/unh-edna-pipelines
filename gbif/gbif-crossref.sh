...
grep -f ${tax} filt_temp_cleaned_${gbifin} | sort | uniq -c  | awk '{print $1}' > counts.tmp
grep -f ${tax} filt_temp_cleaned_${gbifin} | sort | uniq | awk -v FS=',' -v OFS=',' '{print $2,$3,$4,$6,$7,$8}' > taxa.tmp
paste -d',' counts.tmp taxa.tmp > ${out}_family_hit_counts.csv
awk -v FS=',' '$1 > 3 {print $0}' ${out}_family_hit_counts.csv > ${out}_gbif_map_intersect_family.txt
